...
Prepare a portrait photo for clean ASCII conversion:
  1. try to remove the background (rembg) so the subject is isolated
...
try:
    ...
    HAS_REMBG = False
    print(f"Warning: could not import rembg ({e}). Falling back to grayscale + CLAHE without background isolation.")
...
if HAS_REMBG:
    try:
        ...
        print(f"Error while running rembg: {e}. Falling back to grayscale + CLAHE without background isolation.")
        HAS_REMBG = False

if not HAS_REMBG:
    # Fallback: simple grayscale CLAHE
