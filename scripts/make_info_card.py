...
ROWS = [
    ("host",),
    ("kv", "Now", "AI & Data Science Student @ SRM MDU"),
    ("kv", "Building", "ImgSeek (Windows OCR Search Utility)"),
    ("kv", "Goal", "Bridge AI & beautiful UI"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Languages", "Python, C#, JS, C, Java, HTML/CSS"),
    ("kv", "Web", "Node.js, Express, FastAPI"),
    ("kv", "Desktop/.NET", ".NET, WinForms/WPF, C#"),
    ("kv", "DB & Tools", "MongoDB, MySQL, Git"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Building ImgSeek (High-Performance Offline OCR)"),
    ("bul", "Aspiring AI/DS & Frontend Engineer"),
    ("bul", "Reze ma Queen 🥀"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">aaditya079@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">aadi</tspan><tspan fill="{MUTED}">@</tspan>'
