...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "aadi-ascii.svg")
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">aaditya079@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'aaditya079@github:~$ whoami <tspan fill="{INK}">Aaditya Srinivasan</tspan>'
             f'<tspan fill="{INK}"><animate attributeName="opacity" values="1;1;0;0" keyTimes="0;0.5;0.51;1" dur="1s" repeatCount="indefinite"/> █</tspan></text>')
